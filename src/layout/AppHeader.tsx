...
const AppHeader: React.FC = () => {
  ...
  useEffect(() => {
    fetch("/api/admin/me")
      .then((res) => res.json())
      .then((data) => setAdminName(data.data?.name || "Admin"))
    ...
    <header className="sticky top-0 flex w-full border-gray-200 bg-white z-99999 dark:border-gray-800 dark:bg-gray-900 lg:border-b">
      <div className="flex w-full flex-col items-center justify-between lg:flex-row lg:px-6">
        <div className="flex w-full items-center justify-between gap-2 border-b border-gray-200 px-3 py-3 dark:border-gray-800 sm:gap-4 lg:justify-normal lg:border-b-0 lg:px-0 lg:py-4">
          ...
          <p className="ml-auto hidden text-sm font-medium text-gray-700 dark:text-gray-300 lg:block">
            {adminName} (admin user)
          </p>
        </div>

        <div className="flex w-full items-center justify-end px-5 py-3 lg:hidden">
